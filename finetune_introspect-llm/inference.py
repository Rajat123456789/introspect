...
parser.add_argument('--model', type=str, default='introspect-medalpaca/llama-3.3-7b-instruct')
...
def get_response(model, question, seed):
    ...
    elif model.startswith("introspect-medalpaca"):
        inferer = Inferer(
            model_name=model,
            prompt_template="medalpaca/prompt_templates/introspect-medalpaca.json",
            peft=False
        )
        ans = inferer(input=question)
        return ans
